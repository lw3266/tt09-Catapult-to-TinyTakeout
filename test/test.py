...
async def test_project(dut):
    ...
    for i in range(1,1000):
        ...
        x = ((dut.uo_out.value << 8) + dut.uio_out.value)
        dut._log.info(f"input: {dut.ui_in.value} Clock = {i} --> Output: {dut.uo_out.value}{dut.uio_out.value} (dec:{x}) Expected: {expected[1]} or 1111 1111 1010 0001")

    
    
    for i in range(0,256):        
        dut.ui_in.value = i
        await ClockCycles(dut.clk, 89)
        x = ((dut.uo_out.value << 8) + dut.uio_out.value)
